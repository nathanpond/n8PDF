    ...
    private sealed class Reader(byte[] data, long maximumPixels)
    {
        ...
        private void ReadQuantization(int at, int end)
        {
            while (at < end)
            {
                ...
                for (var i = 0; i < 64 && at + (precision == 0 ? 1 : 2) <= end; i++)
                {
                    table[i] = precision == 0 ? data[at] : (data[at] << 8) | data[at + 1];  // (#43)
                    at += precision == 0 ? 1 : 2;
                }
        ...
        private void ReadFrame(int at, int end, bool progressive, bool arithmetic)
        {
            ...
            if (at + 5 >= data.Length)
                throw new ImageFormatException("JPEG frame header runs past the end of the file.");  // (#42)
        ...
        {
            if (at >= end) return at;  // a scan header past the segment reads nothing (#42)
            ...
            // Spectral selection is a raw byte pair; clamped to the sixty-four coefficients it
            // may name, since it indexes the Zigzag table directly (#41).
            var from = Math.Clamp(at < end ? data[at] : 0, 0, 63);
            var to = Math.Clamp(at + 1 < end ? data[at + 1] : 63, 0, 63);
